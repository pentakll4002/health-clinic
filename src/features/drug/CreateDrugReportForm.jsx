...
import { createDrugReport } from './APIDrugs';
...
const CreateDrugReportForm = ({ onCloseModal }) => {
  ...
  const { mutate: createDrugReportMutation, isLoading } = useMutation({
    mutationFn: createDrugReport,
    onSuccess: (data) => {
      toast.success(`Lập báo cáo sử dụng thuốc thành công (${data.total || 0} thuốc)`);
  ...
  function onSubmit(data) {
    ...
    createDrugReportMutation(payload);
  }
  ...
  return (
    <div>
      <div className='w-full pb-4 mb-10 border-b border-grey-transparent'>
        <h2 className='text-xl font-bold'>Lập báo cáo sử dụng thuốc trong tháng (BM8)</h2>
      </div>
...
export default CreateDrugReportForm;
